...
if getattr(sys, 'frozen', False):
    ...
    # The executable's location, for logs and the external 'Softwares' folder.
    RUNTIME_PATH = Path(sys.executable).parent
    # The temporary bundle directory, for the internal 'software_config.json'.
    BUNDLE_PATH = Path(sys._MEIPASS)
    CONFIG_FILE = BUNDLE_PATH / "software_config.json"
else:
    # Running as a standard Python script
    # The script's location is used for everything.
    RUNTIME_PATH = Path(__file__).parent
    CONFIG_FILE = RUNTIME_PATH / "software_config.json"

# The 'Softwares' directory is always expected to be next to the executable or script.
SOFTWARE_DIR = RUNTIME_PATH / "Softwares"
# The log file is always created next to the executable or script.
LOG_PATH = RUNTIME_PATH / f"install_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def log_processes(moment):
    ...
    for proc in psutil.process_iter(['pid', 'name', 'username']):
        try:
            logger.debug(f"  PID: {proc.info['pid']}, Name: {proc.info['name']}, User: {proc.info['username']}")
...
def install_software(name, check_path, installer, args_list, failed_installations, expected_hash=None, cleanup_paths=None, native_log_paths=None, process_name_to_watch=None, process_timeout=900, processes_to_check=None, services_to_stop=None, requires_reboot=False):
    ...
    try:
        ...
        if name in ["Google Chrome", "Microsoft Office", "FortiClient VPN"]:
